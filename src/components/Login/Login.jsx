...
// {Handling Resgister}
   function  handelLogin(info){

    // {to make spin and hiding the button}
  setloading(true)

  axios.post(`https://ecommerce.routemisr.com/api/v1/auth/signin`,info)
  // {incase of sucess}
  .then((res)=> {setloading(false) ,
    ...
    ,navigate('/')})
  // {incase of failur}
  .catch((apiResponse)=> {setapiError(apiResponse.response?.data.message),setloading(false) ,console.log(apiError)})
...
{/* to show the response from the backend */}
  ...
  <span className="font-medium">{apiError}</span> 
    ...
    <h2 className='font-bold text-2xl mb-6 text-emerald-700	'>Login Now</h2>
...
{/* submit button and spinner condition */}
  <button type="submit" className="text-white bg-emerald-700 hover:bg-emerald-800 focus:ring-4 focus:outline-none focus:ring-emerald-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-emerald-600 dark:hover:bg-emerald-700 dark:focus:ring-emerald-800">{loading==true?<i className='fa fa-spinner fa-spin'></i>:"Login"}</button>
  <p className='py-6'>Didnt Have Account Yet?<Link to={"/Register"} ><span className='text-emerald-700 font-semibold mx-1'>Register Now</span></Link></p>
  </form>
